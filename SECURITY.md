...
guix-install partitions disks, writes bootloaders, and sets the root password on
a fresh system, so it runs with a lot of trust. I take reports about it
...
- any logs or config that help me confirm it

Things I'm especially interested in, given what this tool does: anything that
lets a crafted enterprise config server run code or write outside the install
target, weakens how the root/user password is hashed into `/mnt/etc/shadow`, or
authorizes a substitute server the user didn't pick.
...
The guix-install codebase in this repository is in scope. Issues in third-party
dependencies are better reported upstream, though I do want to hear about it if a
dependency issue is exploitable through how guix-install uses it.
